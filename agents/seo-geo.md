...
- **Snippet/AI-visibility controls** (`snippet_controls` per page, from `robots` view - AI features doc: "robots.txt directives for Googlebot" plus `nosnippet`/`data-nosnippet`/`max-snippet` are the site owner's controls for AI inclusion): a `nosnippet` or `max-snippet:0` on a page you'd expect Google to cite in AI Overviews/AI Mode blocks that citation entirely - flag as `high` if it looks unintentional (e.g. present sitewide via a template rather than deliberately on a paywalled/sensitive page). `data_nosnippet_elements` > 0 scopes the restriction to part of a page - note it, don't assume it's wrong. Do not recommend adding these directives; only flag them when they contradict the apparent intent of the page.
